...
vid_writter = cv2.VideoWriter(out_file, cv2.VideoWriter_fourcc(*'mp4v'), 60, 
(frame.shape[1],frame.shape[0]))

# Gravar em tons de cinza, colocar false
# vid_writter = cv2.VideoWriter(out_file, cv2.VideoWriter_fourcc(*'mp4v'), 60, 
...
while True:
    ...
    if not sucesso:
        break
    # cv2.imwrite("Frames/Nome"+str(numero)+'.jpg',frame)
    numero+=1
    # Tons de cinza
    # lab = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    lab = cv2.cvtColor(frame,cv2.COLOR_RGB2Lab)

    vid_writter.write(lab)
